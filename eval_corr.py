...
from eval import get_run_metrics, read_run_dir, get_model_from_run, eval_batch, aggregate_metrics
...
run_id = "pretrained_skew"  # if you train more models, replace with the run_id from the table above
...
# %%
# MSE over examples 
possible_hidden_layer = list(range(1,13)) #[1, ..., 11]
...
with torch.no_grad():
     for i in tqdm(range(n_layers)):
        if i not in possible_hidden_layer:
            continue 
        if 'skew' in run_id:
            model_i, _ = get_model_from_run(f"../models/by_layer_linear_skew/probing_{i}")
        else:
            model_i, _ = get_model_from_run(f"../models/by_layer_linear_standard/probing_{i}")
        readout = model_i._read_out
        hidden_state = output.hidden_states[i]
        pred_at_i = readout(hidden_state)[:,::2,0]
        loss = metric(pred_at_i, ys).detach().numpy()
...
# %%
def plot_correlation(model_name='newton',
    ...
    for k, correlation_func in enumerate(['cosine', 'pearsonr', 'l2']):
        heatmap_first_20 = np.zeros((len(possible_hidden_layer), len(possible_model_params)))
        heatmap_last_20 = np.zeros((len(possible_hidden_layer), len(possible_model_params)))
        ...
        for i, n_layer in enumerate(possible_hidden_layer):
            for j, n_param in enumerate(possible_model_params):
                ...
                if correlation_func == 'cosine':
                    heatmap_first_20[i,j] = np.mean([cosine_similarity(tf_resid[k, :20].reshape(1, -1), 
                                                    method_resid[k,:20].reshape(1, -1)
                                                    ).flatten()[0] for k in range(batch_size)])
                    heatmap_last_20[i,j] = np.mean([cosine_similarity(tf_resid[k, 20:].reshape(1, -1), 
                                                    method_resid[k,20:].reshape(1, -1)
                                                    ).flatten()[0] for k in range(batch_size)])
                    heatmap_all[i,j] = np.mean([cosine_similarity(tf_resid[k, :].reshape(1, -1), 
                                                    method_resid[k,:].reshape(1, -1)
                                                    ).flatten()[0] for k in range(batch_size)])
                elif correlation_func == 'pearsonr':
                    try:
                        heatmap_first_20[i,j] = np.mean([pearsonr(tf_resid[k, :20], method_resid[k,:20])[0] for k in range(batch_size)])
                        heatmap_last_20[i,j] = np.mean([pearsonr(tf_resid[k, 20:], method_resid[k,20:])[0] for k in range(batch_size)])
                        heatmap_all[i,j] = np.mean([pearsonr(tf_resid[k, :], method_resid[k,:])[0] for k in range(batch_size)])
                    except:
                        pdb.set_trace()
                elif correlation_func == 'l2':
                    heatmap_first_20[i,j] = np.mean([np.linalg.norm(tf_resid[k, :20] - method_resid[k,:20], ord=2) / 20 for k in range(batch_size)])
                    heatmap_last_20[i,j] = np.mean([np.linalg.norm(tf_resid[k, 20:] - method_resid[k,20:], ord=2) / 20 for k in range(batch_size)])
                    heatmap_all[i,j] = np.mean([np.linalg.norm(tf_resid[k, :] - method_resid[k,:], ord=2) / 40 for k in range(batch_size)])
                else:
                    raise NotImplementedError()
        
        sns.heatmap(heatmap_first_20.T, annot=True, cmap='plasma_r', \
        ...
        for col, _ in enumerate(heatmap_first_20):
            ...
            ax[0,k].add_patch(Rectangle((col, row),1,1, fill=False, edgecolor='red', lw=3))
        ax[0,k].set_title(f"{correlation_func.upper()} Correlation (v.s. {model_name.upper()}) on First 20 Examples")
        ax[0,k].xaxis.set_ticks_position('top')
        ax[0,k].xaxis.set_label_position('top')
        ax[0,k].tick_params(axis='y', rotation=0)

        sns.heatmap(heatmap_last_20.T, annot=True, cmap='plasma_r', \
                    xticklabels=possible_hidden_layer, yticklabels=possible_model_params,
                    ax=ax[1,k], fmt='.3f')
        for col, _ in enumerate(heatmap_last_20):
            ...
            ax[1,k].add_patch(Rectangle((col, row),1,1, fill=False, edgecolor='red', lw=3))
        ax[1,k].set_title(f"{correlation_func.upper()} Correlation (v.s. {model_name.upper()}) on Last 20 Examples")
        ax[1,k].xaxis.set_ticks_position('top')
        ax[1,k].xaxis.set_label_position('top')
        ax[1,k].tick_params(axis='y', rotation=0)

        sns.heatmap(heatmap_all.T, annot=True, cmap='plasma_r', \
                    xticklabels=possible_hidden_layer, yticklabels=possible_model_params,
                    ax=ax[2,k], fmt='.3f')
        for col, _ in enumerate(heatmap_all):
            ...
            ax[2,k].add_patch(Rectangle((col, row),1,1, fill=False, edgecolor='red', lw=3))
        ax[2,k].set_title(f"{correlation_func.upper()} Correlation (v.s. {model_name.upper()})")
...
# %%
plot_correlation(model_name='gd', possible_model_params=list(range(1, 10, 3)) + list(range(0,80,10))[1:], lr=1e-3)

# %%
plot_correlation(model_name='knn', possible_model_params=range(1,31))
